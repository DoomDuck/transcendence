...
export class SingleNumberPanel {
    ...
    draw(mainContext: CanvasRenderingContext2D, x: number, y: number, size: number) {
        let xTarget = this.drawDirection == Direction.LEFT ? x - this.ratio * size : x;
        mainContext.drawImage(
            this.canvas,
            0, 0, this.width, this.height,
            xTarget, y - size / 2, this.ratio * size, size
        );
    }
}
...
export class ScorePanels {
    ...
    increment(playerId: number) {
        if (playerId == 0)
            this.panel1.increment();
        else
            this.panel2.increment();
    }

    draw(mainContext: CanvasRenderingContext2D, x2: number, y: number, size: number) {
