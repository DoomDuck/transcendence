...
export function applyForces(data: DataBuffer) {
    ...
    data.ballNext.vx = data.ballCurrent.vx + GSettings.GAME_STEP_S * resultForce.x;
    data.ballNext.vy = data.ballCurrent.vy + GSettings.GAME_STEP_S * resultForce.y;
...
export function computeForce(data: DataBuffer, graviton: GravitonData): Vector2 {
    ...
    let d = (data.ballCurrent.y - graviton.y) / GSettings.GRAVITON_FORCE_HEIGHT_HALF;
...
export function applySpeed(data: DataBuffer) {
    data.ballNext.x = data.ballCurrent.x + GSettings.GAME_STEP_S * data.ballNext.vx;
    data.ballNext.y = data.ballCurrent.y + GSettings.GAME_STEP_S * data.ballNext.vy;
    data.barsNext[0].y = data.barsCurrent[0].y + GSettings.GAME_STEP_S * data.barsNext[0].vy;
    data.barsNext[1].y = data.barsCurrent[1].y + GSettings.GAME_STEP_S * data.barsNext[1].vy;
}
