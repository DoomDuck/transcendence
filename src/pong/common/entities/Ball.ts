...
export class Ball {
  ...
  gotOutOfScreen(): boolean {
    if (this.outOfScreenAlreadyReported) return false;
    if (
      this.leftX() > GSettings.GAME_RIGHT ||
      this.rightX() < GSettings.GAME_LEFT
    ) {
  ...
  handleBarCollision(bar: Bar) {
    // detection
    if (this.speed.x * bar.collisionEdgeDirection > 0) return;
    let collision = ballBarCollisionDistanceData(this, bar);
    let distance = collision.distanceVec.length();
    if (!collision.inside && distance >= this.radius) return;
    if (
      !collision.inside &&
    ...
    if (!collision.inside) posCorrection.setLength(this.radius - distance);
    else posCorrection.setLength(this.radius + distance);
    ...
    if (collision.horizontal || collision.corner) {
      ...
      if (collision.corner)
        this.speed.x +=
          Math.sign(this.speed.x) * GSettings.BALL_SPEEDX_CORNER_BOOST;
