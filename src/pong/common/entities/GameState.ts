import { BarInputEvent, GameProducedEvent, SetBallEvent, type DataChangerEvent } from "../game/events";
import { collisions } from "./collisions";
import { DataBuffer } from "./data";
import { applyForces, applySpeed, processExternEvents, propagateBarInputs, updateGravitons } from "./update";
...
export class GameState {
    ...
    handleEvent(event: DataChangerEvent) {
        ...
        if (event.time < this.data.currentTime) {
            console.log(`PAST, event.time = ${event.time}, data.currentTime = ${this.data.currentTime}, data.actualNow = ${this.data.actualNow}`)
            // past event
